...
Run these with `make bench-compare-singlecore` (against every other library) and `make bench-singlecore-vs-sharded` (the two goache implementations head to head). All numbers below are at **`-cpu=1`**, 100,000-entry working set, AMD Ryzen AI 9 HX 370, Go 1.26.2.

You do not have to ask for this implementation to see it in the comparison: at `GOMAXPROCS=1` the `BenchmarkGoache_*` benchmarks construct it themselves ([docs/adr/0029](docs/adr/0029-benchmark-selects-implementation-by-core-count.md)), so the `NewSingleCore` column below is also what the cross-library tables report as "goache" when they are run at one core. The sharded column comes from `BenchmarkGoacheSharded_*`, which forces `New` at any core count.
...
**The `goache` row is whichever implementation a caller on that machine should be using.** `BenchmarkGoache_*` constructs with `NewSingleCore` when `GOMAXPROCS` is 1 and with `New` above it, because "goache" is a library with two implementations and which one is correct is decided by the core count — see [docs/adr/0029](docs/adr/0029-benchmark-selects-implementation-by-core-count.md). Every table in this section ran at 24 threads, so every `goache` cell below is the sharded `Cache`, unchanged. The single-core numbers are in [Single-core mode](#single-core-mode-newsinglecore) above. Either implementation can be forced at any core count via `BenchmarkGoacheSharded_*` and `BenchmarkGoacheSingleCore_*`.
