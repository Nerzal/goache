...
```
BenchmarkSet-24               34777144    34.37 ns/op     0 B/op   0 allocs/op
BenchmarkSetMany-24           10939521   107.5 ns/op    340 B/op   2 allocs/op
BenchmarkGet-24               51261240    23.65 ns/op     0 B/op   0 allocs/op
BenchmarkGetMiss-24           21839464    57.47 ns/op     7 B/op   0 allocs/op
BenchmarkParallelGetSet-24   184743710     6.285 ns/op    0 B/op   0 allocs/op
BenchmarkParallelGet-24     258027836     4.540 ns/op    0 B/op   0 allocs/op
```
...
`ParallelGetSet`/`ParallelGet` dropped ~15-20% from prior numbers (7.366→6.285, 5.474→4.540) after padding each shard to a full cache line in a contiguous `[]shard[K, V]` slice, removing false sharing between adjacent shards' mutexes — see [docs/adr/0018](docs/adr/0018-gemini-analysis-experiments.md), which supersedes [docs/adr/0004](docs/adr/0004-shard-storage-pointer-slice.md)'s earlier (unpadded) measurement. Single-threaded `Set`/`Get` are flat, as expected — false sharing is a multi-core effect.
...
```
BenchmarkFreshLoad_NoHint-24                1286   1002250 ns/op   2518315 B/op   14008 allocs/op
BenchmarkFreshLoad_WithCapacityHint-24      1504    835974 ns/op   2132230 B/op   12749 allocs/op
```
...
```
BenchmarkSetWithTTL-24      28673973    41.80 ns/op     0 B/op   0 allocs/op
BenchmarkGetWithTTL-24      42198394    27.82 ns/op     0 B/op   0 allocs/op
BenchmarkPurge-24                226   5398072 ns/op     0 B/op   0 allocs/op
```
...
```
BenchmarkDelete-24         30161589    38.99 ns/op      0 B/op   0 allocs/op
BenchmarkDeleteMany-24     17361588    67.38 ns/op     84 B/op   1 allocs/op
BenchmarkClear-24                 98  11021529 ns/op   22987145 B/op   106406 allocs/op
```

`Delete` is zero-allocation, same cost class as `Set`. `DeleteMany`'s single alloc/op is the same per-call shard-bucket grouping cost `SetMany` already pays. `Clear`'s number includes populating a fresh 100k-entry cache first (see `BenchmarkFreshLoad_NoHint` above, ~1.0ms) — the `Clear()` call itself only accounts for a fraction of the ~11.0ms total; the rest, including essentially all 106406 allocs/op, is the `SetMany` population that precedes it in the benchmark (see "Automatic eviction" below for why every new key now allocates). Note `Delete`/`DeleteMany`/`Clear` here all run against an *unbounded* cache (no `WithMaxSize`) — they deliberately never touch the entry pool described below, since nothing on the unbounded path ever draws from it (see [docs/adr/0018](docs/adr/0018-gemini-analysis-experiments.md)).
...
```
BenchmarkSetWithMaxSize-24   15540778    76.00 ns/op     0 B/op   0 allocs/op
BenchmarkGetWithMaxSize-24   45966795    26.48 ns/op     0 B/op   0 allocs/op
BenchmarkEvictionChurn-24    10666078   115.0 ns/op      8 B/op   1 allocs/op
BenchmarkParallelGetWithMaxSize-24 251821138   4.753 ns/op   0 B/op   0 allocs/op
```

![Eviction cost benchmark chart](docs/img/eviction-cost.svg)

`GetWithMaxSize` is barely different from plain `Get` (26.48 vs 23.65 ns/op) — the only added cost is one atomic bit store; `ParallelGetWithMaxSize` confirms this holds under concurrency too (4.753 ns/op, in line with plain `ParallelGet`'s 4.540). `BenchmarkSetWithMaxSize` (cache capped at half the key pool, so roughly every other `Set` evicts) and `BenchmarkEvictionChurn` (cache always full, *every* `Set` evicts) show the cost of enabling eviction: 76.00 and 115.0 ns/op respectively, both down substantially (~40% and ~31%) from earlier numbers (130.8 and 174.9 ns/op) after entries on `WithMaxSize`-bounded shards started recycling through a `sync.Pool` instead of always allocating fresh on every eviction-driven insert — allocs/op dropped from 1→0 and 2→1 accordingly. See [docs/adr/0018](docs/adr/0018-gemini-analysis-experiments.md) for the full measurement, including why this pooling is deliberately *not* applied to unbounded caches (it regressed `Delete`/`Clear`/`SetMany`/`FreshLoad_*` there in an earlier, cache-wide attempt, since nothing ever draws those entries back out of a pool that's never drained by eviction).
...
| goache | 93.27 | 105.1 | 118.9 | 123.7 | 220.5 |
...
| Library | 1,000 | 5,000 | 50,000 | 100,000 | 1,000,000 |
|---|---|---|---|---|---|
| goache | 45.79 | 60.17 | 68.70 | 111.4 | 250.2 |
